...
class GraphQA:
    def __init__(self):
        self.graph = Graph("http://127.0.0.1:7474/browser/", auth=("neo4j", "1234567"))
...
##########################修改部分##########################################################
            if isinstance(value, dict):
                result = '。'.join('{}:{}'.format(k,v) for k,v in value.items())
                value = result
########################################################################################
    ...
    def parse_result(self, graph_search_result, answer, info):
        ...
        if "REL" in graph_search_result:
            # print(list(graph_search_result["REL"].types()))
...
if __name__ == "__main__":
    graph = GraphQA()
    #############新增部分##########################
    res = graph.query("阿爸是谁填词的")
    print(res)
    res = graph.query("爱在西元前的详细信息")
    print(res)
    res = graph.query("谁是听妈妈的话的MV导演")
    print(res)
    ###############################################
