...
'''
Author: Shiyao Ma
Date: 2023-11-17 13:20:56
LastEditors: Shiyao Ma
LastEditTime: 2023-11-17 17:03:22
...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    for similar_item, similarity in similar_items[item_id][:topn]:
        # 相似用户对这部电影的打分
        rating_similar_item_by_user = user_to_rating[user_id][similar_item - 1]
        # 分数*用户相似度，作为一种对分数的加权，越相似的用户评分越重要
        pred_score += rating_similar_item_by_user * similarity
        # 如果这个相似用户没看过，就不计算在总数内
        if rating_similar_item_by_user != 0:
            count += 1
    pred_score /= count + 1e-5
    return pred_score
...
def movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name, topn=10):
    ...
    # seen_items = [item_id + 1 for item_id, rating in enumerate(user_to_rating[user_id]) if rating != 0]
    res = []
