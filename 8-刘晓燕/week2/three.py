...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        # ce_loss = nn.functional.cross_entropy  # 交叉熵

        self.activation = torch.sigmoid  # sigmoid归一化函数
        self.loss = nn.functional.binary_cross_entropy  # loss函数采用 交叉熵 损失
...
# 随机生成一个3维向量，如果第一个值大于第五个值，认为是正样本，反之为负样本
def build_sample():
...
def evaluate(model):
    ...
    print("其中有样本一: %d 个; 样本二: %d 个; 样本三: %d 个" % (
        sum(y[:, 0]), sum(y[:, 1]), sum(y[:, 2])))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        y_pred_label = torch.max(y_pred, 1)[1]
