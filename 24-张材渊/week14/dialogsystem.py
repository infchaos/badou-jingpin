...
class DialogSystem:
    ...
    def load(self):
        ...
        self.normal_node_id = []  # [常规节点id]
        self.load_scenrio("scenario-买衣服.json")
        self.load_scenrio("scenario-重听.json") # 常规节点
    ...
    def load_scenrio(self, path):
        ...
        for node_info in scenario_data:
            ...
            if node_info.get("action") == 'normal':
                # 常规节点id存储
                self.normal_node_id.append(node_id)
    ...
    def get_intent(self, memory):
        ...
        hit_node = None
        hit_score = -1
        for node_id in memory["available_node"]+self.normal_node_id:
            score = self.get_node_score(node_id, memory)
            if score > hit_score:
                hit_node = node_id
                hit_score = score
        if hit_node not in self.normal_node_id:
